...
export function pluginRootFromImportMeta(importMetaUrl) {
  return path.resolve(path.dirname(fileURLToPath(importMetaUrl)), "..");
}

export function pythonCandidates(scriptPath) {
...
export function runPythonMcp({ pluginRoot, scriptPath }) {
  const candidates = pythonCandidates(scriptPath);
  const failures = [];
  ...
  function spawnCandidate(candidate) {
    ...
    const child = spawn(candidate.command, candidate.args, {
      ...
      env: {
        ...
        AGENTOS_NODE_LAUNCHER: fileURLToPath(import.meta.url),
      },
